...
def get_chatbot_response(question):
    ...
    for attempt in range(retry_attempts):
        try:
            response = requests.post('http://127.0.0.1:5000/ask', json={"message": question}, timeout=10)
            response.raise_for_status()  # Raises a HTTPError for bad responses (4xx and 5xx)
            return response.json().get("response", "")
        except requests.exceptions.RequestException as e:
            print(f"Request failed: {e}, attempt {attempt + 1}/{retry_attempts}")
...
def evaluate_test_case(test_case):
    ...
    relevancy_metric = AnswerRelevancyMetric(threshold=0.9, model="gpt-3.5-turbo", include_reason=True)
    faithfulness_metric = FaithfulnessMetric(threshold=0.9, model="gpt-3.5-turbo", include_reason=True)
...
def run_tests():
    ...
    with open("results_deep.json", "w") as results_file:
        json.dump(results, results_file, indent=4)

    print("\nEvaluation completed. Results saved to 'results_deep.json'.")
